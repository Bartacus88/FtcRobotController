...
/*                else if ((gamepad1.dpad_up) || (turtleInitiated)) {
            ...
            if (CurrentTime - LastNav > NAVPERIOD) {
                ...
                // CRAB STYLE DRIVE
                //FORWARD/REVERSE
                if (Math.abs(gamepad1.right_stick_x) > g1_Crab_Threshold) {  //Forward, backward
                    leftDriveCrab = gamepadDrive.condition(gamepad1.right_stick_x, GamepadDrive.CONDITION_TYPE.TURTLE_HIGH_MANEUVERABILITY);
                ...
                if (Math.abs(gamepad1.right_stick_y) > g1_Crab_Threshold) {
                    leftDriveCrab = -1 * gamepadDrive.condition(gamepad1.right_stick_y, GamepadDrive.CONDITION_TYPE.TURTLE_HIGH_MANEUVERABILITY);
